...
__export(chains_exports, {
  ...
  defiOracleMetaMainnet: () => defiOracleMetaMainnet,
  ...
  marooTestnet: () => marooTestnet,
  marsCredit: () => marsCredit,
  matchain: () => matchain,
  ...
  morphTachyon: () => morphTachyon,
  morphTachyonTestnet: () => morphTachyonTestnet,
  nahmii: () => nahmii,
  nautilus: () => nautilus,
  near: () => near,
...
// node_modules/viem/_esm/chains/definitions/hausTestnet.js
var hausTestnet = defineChain({
  id: 2443,
...
// node_modules/viem/_esm/chains/definitions/morphTachyon.js
var morphTachyon = defineChain({
  id: 2184,
  name: "Morph Tachyon",
  nativeCurrency: {
    name: "BGB",
...
// node_modules/viem/_esm/chains/definitions/morphTachyonTestnet.js
var morphTachyonTestnet = defineChain({
  id: 34952,
...
var maxOwners = 50;
var maxThreshold = 8;
var maxSignatures = maxThreshold;
var maxNestingDepth = 2;
...
function getAddress(config) {
  assert4(config);
  const hash5 = keccak2562(concat2(fromString(accountDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber2(config.threshold, { size: 1 }), fromNumber2(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [
    owner.owner,
    fromNumber2(owner.weight, { size: 1 })
  ])));
  const account = from4(slice2(hash5, 12, 32));
  if (toBigInt2(account) === 0n)
    throw new InvalidConfigError({ reason: "derived account cannot be zero" });
  return account;
}
function getSignPayload(value) {
  const { payload, version: version2 = 0n } = value;
  const account = "account" in value && value.account ? value.account : getAddress(value.initialConfig);
  return keccak2562(concat2(fromString(signatureDomain), from2(payload), account, fromNumber2(version2, { size: 8 })));
}
...
function assertMultisig(envelope, depth) {
  ...
  if (!validate3(envelope.account))
    throw new InvalidMultisigApprovalError({
      reason: "multisig account is invalid"
    });
  if (toBigInt2(envelope.account) === 0n)
    throw new InvalidMultisigApprovalError({
  ...
  if (envelope.signatures.length > maxSignatures)
    throw new InvalidMultisigApprovalError({
      reason: `multisig signatures exceed ${maxSignatures}`
    });
  if (envelope.init) {
    assert4(envelope.init);
    if (!isEqual(getAddress(envelope.init), envelope.account))
      throw new InvalidMultisigApprovalError({
        reason: "multisig init does not derive account"
      });
  }
  for (const inner of envelope.signatures) {
    const type2 = getType(inner);
    if (type2 === "keychain")
      throw new InvalidMultisigApprovalError({
    ...
    if (type2 === "multisig") {
      ...
      assert5(inner);
      if (size(serialize(inner)) > maxOwnerSignatureBytes)
        throw new InvalidMultisigApprovalError({
...
function deserialize_(value, multisigDepth) {
  ...
  if (typeId === serializedKeychainType || typeId === serializedKeychainV2Type) {
    ...
    const inner = deserialize_(slice2(data, 20), multisigDepth);
    return {
      userAddress,
      inner,
      type: "keychain",
  ...
  if (typeId === serializedMultisigType) {
    ...
    for (const signature of signatures)
      if (size(signature) > maxOwnerSignatureBytes && slice2(signature, 0, 1) !== signatureTypeByte)
        throw new InvalidSerializedError({
          reason: `multisig owner signature exceeds ${maxOwnerSignatureBytes} bytes`,
          serialized
        });
    if (!Array.isArray(address2) && !validate3(address2))
      throw new InvalidSerializedError({
    ...
    if (Array.isArray(address2)) {
      ...
      if (address2.length !== 3 || Array.isArray(salt) || size(salt) !== 32 || Array.isArray(threshold) || size(threshold) > 1 || !Array.isArray(owners) || owners.some((owner) => !Array.isArray(owner) || owner.length !== 2 || owner.some(Array.isArray) || size(owner[1]) > 1))
        throw new InvalidSerializedError({
          reason: "invalid multisig init config",
          serialized
        });
    }
    const init = Array.isArray(address2) ? fromTuple2(address2) : void 0;
    const account = init ? getAddress(init) : address2;
    const envelope = {
      type: "multisig",
      account,
  ...
  if (type2 === "multisig") {
    ...
    const account = (() => {
      if (rest.account)
        return rest.account;
      if (initialConfig)
        return getAddress(initialConfig);
      return rest.account;
    })();
    const initSource = init === true ? initialConfig : init || void 0;
    return {
      ...rest,
      account,
...
function fromRpc3(envelope) {
  ...
  if (envelope.type === "multisig" || "signatures" in envelope && ("account" in envelope || "init" in envelope)) {
    ...
    const init = hasInit ? from8(multisig.init) : void 0;
    const account = init ? getAddress(init) : multisig.account;
    const result = {
      type: "multisig",
      account,
      signatures: multisig.signatures.map((signature) => fromRpc3(signature)),
      ...init ? { init } : {}
    };
    assert5(result);
    return result;
...
function getType(envelope) {
  ...
  if ("userAddress" in envelope && "inner" in envelope)
    return "keychain";
  if (("account" in envelope || "initialConfig" in envelope || "init" in envelope) && "signatures" in envelope)
    return "multisig";
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "multisig") {
    ...
    return concat2(serializedMultisigType, fromHex3([
      multisig.init ? toTuple2(multisig.init) : multisig.account,
...
function sortMultisigApprovals(value) {
  const { payload, signatures, version: version2 = 0n } = value;
  const digest = getSignPayload("initialConfig" in value && value.initialConfig ? { payload, initialConfig: value.initialConfig, version: version2 } : {
    payload,
...
function toRpc3(envelope) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    assert5(multisig);
    const signatures = multisig.signatures.map((signature) => toRpc3(signature));
    if (multisig.init) {
      const init = {
        ...multisig.init,
        salt: multisig.init.salt ?? zeroSalt,
...
// node_modules/ox/_esm/tempo/EarnShares.js
var EarnShares_exports = {};
__export(EarnShares_exports, {
  InvalidExpectedOutputError: () => InvalidExpectedOutputError,
  InvalidSlippageError: () => InvalidSlippageError,
  basisPointScale: () => basisPointScale,
  feeShares: () => feeShares,
  minimumOutput: () => minimumOutput,
  toAmount: () => toAmount,
  toAmountUp: () => toAmountUp,
  toVenueAmount: () => toVenueAmount
});
var basisPointScale = 1e4;
...
function minimumOutput(expectedAmount, slippageBps) {
  ...
  if (!Number.isInteger(slippageBps) || slippageBps < 0 || slippageBps >= basisPointScale)
...
var InvalidExpectedOutputError = class extends BaseError2 {
  constructor(options) {
...
var InvalidSlippageError = class extends BaseError2 {
  constructor(options) {
...
__export(KeyAuthorization_exports, {
  ...
  InvalidSignatureTypeError: () => InvalidSignatureTypeError,
...
function from13(authorization, options = {}) {
  ...
  if (options.signature) {
    const signature = from9(options.signature);
...
function fromRpc5(authorization) {
  ...
  const accountBinding = account !== void 0 ? { account, isAdmin: isAdmin2 ?? false } : {};
  return {
    address: keyId,
...
function fromTuple4(tuple) {
  ...
  if (signatureSerialized) {
    const signature = deserialize(signatureSerialized);
    assertSignature(signature);
...
function toTuple4(authorization) {
  ...
  const signature = (() => {
    ...
    return serialize(authorization.signature);
  })();
  const type2 = (() => {
...
var InvalidSignatureTypeError = class extends Error {
  constructor(type2) {
    super(`Signature type \`${type2}\` is invalid for key authorizations; expected \`secp256k1\`, \`p256\`, \`webAuthn\`, or \`multisig\`.`);
    Object.defineProperty(this, "name", {
      enumerable: true,
      configurable: true,
      writable: true,
...
var transferBlocked = from7("event TransferBlocked(address indexed token, address indexed receiver, uint64 indexed blockedNonce, uint256 amount, uint8 receiptVersion, bytes receipt)");
...
function from16(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from16(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from16(gwei, exponents.gwei - exponents[unit]);
}
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload4(from17(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload4(from17(envelope)),
...
__export(ZoneId_exports, {
  UnsupportedSourceIdError: () => UnsupportedSourceIdError,
...
var chainIdBase = chainIdConfig[defaultSourceId].base;
function fromChainId(chainId, sourceId70 = defaultSourceId) {
  return chainId - getChainIdConfig(sourceId70).base;
}
function toChainId(zoneId, sourceId70 = defaultSourceId) {
  const { base: base2, range } = getChainIdConfig(sourceId70);
  return base2 + zoneId % range;
}
var UnsupportedSourceIdError = class extends BaseError2 {
  constructor({ sourceId: sourceId70 }) {
    super(`Source chain ID "${sourceId70}" is not supported.`, {
...
function getChainIdConfig(sourceId70) {
  if (sourceId70 === 4217 || sourceId70 === 42431)
    return chainIdConfig[sourceId70];
  throw new UnsupportedSourceIdError({ sourceId: sourceId70 });
}
...
var fieldsSize = 29;
var version = 0;
function from19(authentication, options = {}) {
...
var zone = [...zoneMessenger, ...zoneOutbox, ...zoneVerifier];
var all = [...core, ...earn, ...zone];
...
async function serializeTempo(transaction, sig) {
  ...
  const hasSenderSignature = typeof signature_provided !== "undefined" || Boolean(transaction.multisig && transaction.signatures);
  const shouldStripFeeTokenForSponsorship = (
    // Relay fills a partial sender envelope first.
    feePayer === true && (!hasSenderSignature || !feePayerSignature) || // Local fee-payer accounts sign after sender serialization.
    typeof feePayer === "object" && !hasSenderSignature || // Filled transactions can already include fee-payer approval.
  ...
  const signature = (() => {
    ...
    const sorted = typeof transaction.multisig === "string" ? SignatureEnvelope_exports.sortMultisigApprovals({
      account: transaction.multisig,
      payload,
      signatures,
      version: transaction.multisigVersion
    }) : SignatureEnvelope_exports.sortMultisigApprovals({
      initialConfig: transaction.multisig,
      payload,
      signatures,
      version: transaction.multisigVersion
    });
    const keyAuthorizationSignature = (_a3 = transaction.keyAuthorization) == null ? void 0 : _a3.signature;
    const keyAuthorizationInitializes = (keyAuthorizationSignature == null ? void 0 : keyAuthorizationSignature.type) === "multisig" && typeof keyAuthorizationSignature.init !== "undefined";
    ...
    return SignatureEnvelope_exports.from({
      initialConfig: transaction.multisig,
      signatures: sorted,
      ...nonce2 || transaction.nonceKey || keyAuthorizationInitializes ? {} : { init: true }
    });
  })();
...
async function signMultisig(account, parameters2) {
  const { init = false, payload, signatures: providedSignatures = [], states = [], version: version2 = 0n } = parameters2;
  const state = states.find((state2) => isEqual(state2.account, account.address));
  const currentConfig = (state == null ? void 0 : state.config) ?? account.config;
  if (!currentConfig)
  ...
  for (const signature of providedSignatures) {
    const address2 = SignatureEnvelope_exports.extractAddress({
      payload: digest,
      signature
    }).toLowerCase();
    const owner = currentConfig.owners.find((owner2) => isEqual(owner2.owner, address2));
    if (!owner || signedOwners.has(address2))
      continue;
    signedOwners.add(address2);
    weight += Number(owner.weight);
  }
  for (const owner of currentConfig.owners) {
    const address2 = owner.owner.toLowerCase();
    if (signedOwners.has(address2))
      continue;
    const ownerAccount = account.owners.find((account2) => isEqual(account2.address, owner.owner));
    if (!ownerAccount)
      continue;
    if (isMultisigAccount(ownerAccount)) {
      const state2 = states.find((state3) => isEqual(state3.account, ownerAccount.address));
      if (state2 && !state2.initialized)
        throw new Error("Nested multisig owners must be initialized.");
      signatures.push(await signMultisig(ownerAccount, {
        payload: digest,
        states,
        version: state2 == null ? void 0 : state2.version
      }));
    } else {
      if (!ownerAccount.sign)
        throw new Error("Multisig owner account cannot sign.");
      signatures.push(SignatureEnvelope_exports.from(await ownerAccount.sign({ hash: digest })));
    }
    signedOwners.add(address2);
    weight += Number(owner.weight);
  ...
  return SignatureEnvelope_exports.from({
    ...
    signatures: SignatureEnvelope_exports.sortMultisigApprovals({
      account: account.address,
      payload,
      signatures,
      version: version2
...
function getKeyAuthorizationSignPayload(account, parameters2) {
  const { admin, chainId, expiry, key, limits, scopes, witness } = parameters2;
  ...
  return KeyAuthorization_exports.getSignPayload({
...
async function signKeyAuthorization(account, parameters2) {
  const { admin, chainId, expiry, key, limits, multisig: multisigState, scopes, signatures, witness } = parameters2;
  const { accessKeyAddress, keyType: type2 } = resolveAccessKey(key);
  ...
  const restrictions = admin ? {} : { expiry, limits, scopes };
  const hash5 = getKeyAuthorizationSignPayload(account, parameters2);
  const signature = await (async () => {
    if (isAccessKey)
      return account.sign({ hash: hash5, raw: true });
    if (isMultisig) {
      if (!multisigState)
        throw new Error("Multisig state is required to sign a key authorization.");
      return SignatureEnvelope_exports.serialize(await signMultisig(account, {
        init: multisigState.init,
        payload: hash5,
        signatures: signatures == null ? void 0 : signatures.map((signature2) => SignatureEnvelope_exports.from(signature2)),
...
var mainnetZoneChainIdBase = ZoneId_exports.toChainId(0);
var testnetZoneChainIdBase = ZoneId_exports.toChainId(0, 42431);
var zoneChainIdLimit = 2 ** 31;
var accountKeychain2 = "0xaAAAaaAA00000000000000000000000000000000";
var nativeMultisig2 = "0xAACC000000000000000000000000000000000000";
...
// node_modules/viem/_esm/tempo/internal/multisig.js
function createMultisigStateResolver(getConfig2) {
  const cache = /* @__PURE__ */ new Map();
  return (account) => {
    const address2 = account.toLowerCase();
    const cached = cache.get(address2);
    if (cached)
      return cached;
    const state = (async () => {
      var _a2;
      try {
        const config = await getConfig2(account);
        return {
          account,
        ...
        const cause = error instanceof BaseError ? error.walk((error2) => error2 instanceof ContractFunctionRevertedError) : void 0;
        if (!(cause instanceof ContractFunctionRevertedError) || ((_a2 = cause.data) == null ? void 0 : _a2.errorName) !== "NotMultisigAccount")
          throw error;
        return { account, initialized: false, version: 0n };
      }
    })();
    cache.set(address2, state);
    return state;
  };
}
function getMultisigOwnerStates(account, getState) {
  const owners = [account];
  const seen = /* @__PURE__ */ new Set([account.address.toLowerCase()]);
  const visit = (account2) => {
    for (const owner of account2.owners) {
      if (owner.source !== "multisig")
        continue;
      const address2 = owner.address.toLowerCase();
      if (seen.has(address2))
        continue;
      seen.add(address2);
      owners.push(owner);
      visit(owner);
...
// node_modules/viem/_esm/tempo/actions/multisig.js
async function isInitialized(client, parameters2) {
  const { account, ...rest } = parameters2;
...
(function(isInitialized2) {
  function call(args) {
    return defineCall({
      address: nativeMultisig2,
      abi: nativeMultisig,
      args: [args.account],
      functionName: "isMultisigAccount"
    });
  }
  isInitialized2.call = call;
})(isInitialized || (isInitialized = {}));
async function getConfig(client, parameters2) {
  const { account, ...rest } = parameters2;
  return readContract(client, {
    ...rest,
...
(function(getConfig2) {
  ...
  getConfig2.call = call;
})(getConfig || (getConfig = {}));
async function updateConfig(client, parameters2) {
  return updateConfig.inner(writeContract, client, parameters2);
}
(function(updateConfig2) {
  async function inner(action, client, parameters2) {
    const { owners, threshold, ...rest } = parameters2;
    return await action(client, {
      ...rest,
      ...updateConfig2.call({ owners, threshold })
    });
  }
  updateConfig2.inner = inner;
  function call(args) {
    const config = MultisigConfig_exports.from(args);
    return defineCall({
      address: nativeMultisig2,
      abi: nativeMultisig,
  ...
  updateConfig2.extractEvent = extractEvent;
...
async function prepareAuthorization(client, parameters2) {
  var _a2;
  const { chainId = (_a2 = client.chain) == null ? void 0 : _a2.id } = parameters2;
  const account_ = parameters2.account ?? client.account;
  if (!account_)
    throw new Error("account is required.");
  if (!chainId)
    throw new Error("chainId is required.");
  ...
  const multisigState = await (async () => {
    if ("multisig" in parameters2)
      return parameters2.multisig;
    if (parsed.source !== "multisig")
      return void 0;
    const account = parsed;
    const getState = createMultisigStateResolver((account2) => getConfig(client, { account: account2 }));
  ...
  const authorizationSignPayload = getKeyAuthorizationSignPayload(parsed, {
    ...
    key: parameters2.accessKey
  });
  const signPayload = parsed.source === "multisig" && multisigState ? MultisigConfig_exports.getSignPayload({
...
async function signAuthorization(client, parameters2) {
  const prepared = "multisig" in parameters2 ? parameters2 : await prepareAuthorization(client, parameters2);
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { client, phase }) => {
      ...
      const multisigIdentity = request.multisig ?? (((_c = request.account) == null ? void 0 : _c.source) === "multisig" ? request.account.config ?? request.account.address : void 0);
      if (multisigIdentity) {
        const initialConfig = typeof multisigIdentity === "string" ? void 0 : MultisigConfig_exports.from(multisigIdentity);
        const multisigAddress = typeof multisigIdentity === "string" ? multisigIdentity : MultisigConfig_exports.getAddress(initialConfig);
        ...
        const getState = createMultisigStateResolver((account) => getAction(client, getConfig, "getConfig")({ account }));
        const ownerStates = ((_d = request.account) == null ? void 0 : _d.source) === "multisig" ? getMultisigOwnerStates(request.account, getState) : void 0;
        const state = ownerStates ? (await ownerStates)[0] : await getState(multisigAddress);
        ...
        const authorizationSignature = (_e = request.keyAuthorization) == null ? void 0 : _e.signature;
        if ((authorizationSignature == null ? void 0 : authorizationSignature.type) === "multisig" && typeof authorizationSignature.init !== "undefined")
        ...
        const keyAuthorizationSignature = (_f = request.keyAuthorization) == null ? void 0 : _f.signature;
        const keyAuthorizationInitializes = (keyAuthorizationSignature == null ? void 0 : keyAuthorizationSignature.type) === "multisig" && typeof keyAuthorizationSignature.init !== "undefined";
        if (initialConfig && !keyAuthorizationInitializes)
          request.multisigInit = {
            salt: initialConfig.salt ?? MultisigConfig_exports.zeroSalt,
        ...
        if (((_g = request.account) == null ? void 0 : _g.source) !== "multisig")
      ...
      const useExpiringNonce = await (async () => {
        ...
        const address2 = typeof account === "string" ? account : account == null ? void 0 : account.address;
        if (address2 && typeof request.nonceKey === "undefined")
          return await detect(address2.toLowerCase());
        return false;
      })();
      if (useExpiringNonce) {
      ...
      if (!request.keyAuthorization && ((_h = request.account) == null ? void 0 : _h.source) === "accessKey") {
        const keyAuthorizationManager = request.account.keyAuthorizationManager;
        if (keyAuthorizationManager) {
          const chainId = request.chainId ?? ((_i = request.chain) == null ? void 0 : _i.id);
      ...
      if (!request.feeToken && ((_j = request.chain) == null ? void 0 : _j.feeToken))
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        ...
        const keyInfo = await getMetadata(client, {
          ...
          blockHash: parameters2.blockHash,
          blockNumber: parameters2.blockNumber,
          blockTag: parameters2.blockTag,
          requireCanonical: parameters2.requireCanonical
        });
...
// node_modules/viem/_esm/chains/definitions/ynxTestnet.js
var ynxTestnet = defineChain({
  id: 6423,
  name: "YNX Testnet",
  nativeCurrency: {
    decimals: 18,
...
//# sourceMappingURL=chunk-Y4MN7SP4.js.map
